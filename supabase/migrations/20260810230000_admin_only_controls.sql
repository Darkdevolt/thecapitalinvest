...
create or replace function private.tc_is_admin()
returns boolean
...
begin
  foreach t in array tables loop
    if to_regclass('public.' || t) is not null then
      ...
      execute format('create policy "tc_admin_insert_%s" on public.%I for insert to authenticated with check ((select private.tc_is_admin()))', t, t);
      execute format('drop policy if exists "tc_admin_update_%s" on public.%I', t, t);
      execute format('create policy "tc_admin_update_%s" on public.%I for update to authenticated using ((select private.tc_is_admin())) with check ((select private.tc_is_admin()))', t, t);
      execute format('drop policy if exists "tc_admin_delete_%s" on public.%I', t, t);
      execute format('create policy "tc_admin_delete_%s" on public.%I for delete to authenticated using ((select private.tc_is_admin()))', t, t);
    end if;
  end loop;
end $$;
...
begin
  if to_regclass('public.users') is not null then
    ...
    create policy "tc_user_read_self" on public.users for select to authenticated using (id = auth.uid() or (select private.tc_is_admin()));
    ...
    create policy "tc_admin_update_users" on public.users for update to authenticated using ((select private.tc_is_admin())) with check ((select private.tc_is_admin()));
    drop policy if exists "tc_admin_delete_users" on public.users;
    create policy "tc_admin_delete_users" on public.users for delete to authenticated using ((select private.tc_is_admin()));
  end if;
end $$;
