-- Centralized PER history refresh logic.
-- Full refresh is used for first population and corrections; normal reads only
-- refresh the current year and the immediately previous year so closed years stay frozen.
...
begin
  ...
  target_years as (
    ...
    where p_full or (select n from existing) = 0 or y.annee >= v_current_year - 1
  ),
  latest_courses as (
  ...
  source_rows as (
    select
      ...
      c.cours_reference,
      case
        when b.bpa is null then null
        when b.bpa = 0 then null
        when b.bpa < 0 then null
        when c.cours_reference is null then null
  ...
  upserted as (
    ...
    where p_full
       or per_history.annee >= v_current_year - 1
       or not exists (
         select 1 from public.per_history ph2
         where ph2.ticker = per_history.ticker and ph2.annee = per_history.annee
       )
    returning 1
  )
