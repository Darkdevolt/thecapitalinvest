...
(function (TC) {
    ...
    /* ------------------------------------------------------------
       Découverte du schéma réel.

       Déclarer les colonnes à la main ne suffit pas : la déclaration
       finit toujours par diverger de la base. Une lecture d'une ligne
       suffit à connaître les colonnes véritables, PostgREST renvoyant
       l'enregistrement complet. Le résultat est mis en cache pour la
       session et fait autorité sur TC.COLUMNS.

       Table vide : la découverte ne renvoie rien et l'on retombe sur la
       liste déclarée, volontairement restreinte aux colonnes vérifiées.
       ------------------------------------------------------------ */

    const schemaCache = {};

    TC.schema = async function (table) {
        if (Object.prototype.hasOwnProperty.call(schemaCache, table)) return schemaCache[table];
        const rows = await TC.get(table, 'select=*&limit=1');
        schemaCache[table] = (rows && rows.length) ? Object.keys(rows[0]) : null;
        return schemaCache[table];
    };

    /** Vrai si la colonne existe réellement ; null tant que rien n'est connu. */
    TC.hasColumn = function (table, column) {
        const discovered = schemaCache[table];
        if (discovered) return discovered.indexOf(column) !== -1;
        const declared = TC.COLUMNS[table];
        return declared ? declared.indexOf(column) !== -1 : null;
    };

    TC.forgetSchema = function (table) {
        if (table) delete schemaCache[table]; else Object.keys(schemaCache).forEach(k => delete schemaCache[k]);
    };

    /**
     * Retire les colonnes absentes de la table et les valeurs vides.
     * Les champs écartés alors qu'ils portaient une valeur sont retournés :
     * une donnée saisie par l'utilisateur ne doit jamais disparaître en
     * silence parce que la colonne n'existe pas.
     */
    function fitDetailed(row, table) {
        const allowed = schemaCache[table] || TC.COLUMNS[table];
        if (!allowed) return { row, dropped: [] };
        const out = {};
        const dropped = [];
        Object.keys(row).forEach(function (col) {
            const value = row[col];
            if (value === undefined || value === '') return;
            if (allowed.indexOf(col) !== -1) out[col] = value;
            else if (value !== null) dropped.push(col);
        });
        return { row: out, dropped };
    }

    const fit = TC.fit = function (row, table) {
        return fitDetailed(row, table).row;
    };

    TC.post = async function (table, body, onConflict) {
        await TC.schema(table);
        const sample = Array.isArray(body) ? body[0] : body;
        const dropped = sample ? fitDetailed(sample, table).dropped : [];
        if (dropped.length) {
            TC.toast('Colonnes absentes de la table ' + table + ', non enregistrées : ' +
                dropped.join(', '), 'warn', 7000);
        }
    ...
    TC.patch = async function (table, filter, body) {
        await TC.schema(table);
        const dropped = fitDetailed(body, table).dropped;
        if (dropped.length) {
            TC.toast('Colonnes absentes de la table ' + table + ', non enregistrées : ' +
                dropped.join(', '), 'warn', 7000);
        }
    ...
    /* ── Balayages d'historique, bornés et mutualisés ────── */

    const scans = {};

    /** Début des balayages : aujourd'hui moins TC.SCAN_MONTHS mois. */
    TC.scanFloor = function () {
        const d = new Date();
        d.setMonth(d.getMonth() - (TC.SCAN_MONTHS || 18));
        return d.toISOString().slice(0, 10);
    };

    /**
     * Un même balayage demandé par le tableau de bord, le diagnostic et le
     * calendrier ne doit s'exécuter qu'une fois par session. C'est la promesse
     * qui est mise en cache, pas seulement son résultat : trois appels
     * simultanés partagent une requête au lieu d'en lancer trois.
     */
    TC.scan = function (key, table, query) {
        if (!scans[key]) {
            scans[key] = TC.getAll(table, query).catch(function (e) {
                delete scans[key];
                throw e;
            });
        }
