...
async function analyzeAfterUpload(file,date){if(window.TCBocImporter&&typeof window.TCBocImporter.analyze==='function'){try{await window.TCBocImporter.analyze(file,date);return true;}catch(e){console.error('[BOC analyze]',e);msg('Document enregistré, mais analyse impossible : '+e.message,'err');return false;}}window.__tcPendingBoc={file:file,date:date};return false;}
async function upload(){
 ...
 if(!date)date=parseDate(file.name);if(!date)return msg('Indiquez la date de séance.','err');document.getElementById('boc-date').value=date;
 try{
   ...
   msg('✓ Document enregistré. Analyse du BOC en cours…','ok');
   await analyzeAfterUpload(file,date);
