...
async function hqLoadRows(){
  var rows=await sbGet('historique','select=ticker,date_seance,cours_cloture,cours_ouverture,plus_haut,plus_bas,volume,variation&id=not.is.null&order=date_seance.desc&limit=5000');
...
function hqAnalyse(rows,universe){
  ...
  rows.forEach(function(r){
    ...
    var key=d+'|'+String(r.ticker||'').toUpperCase();
    if(seen[key])duplicate++; else seen[key]=1;
  });
...
async function loadHistoriqueQualityDashboard(){
  ...
  try{
    ...
    if(a.duplicate)alerts.push(['warning','Doublons potentiels',a.duplicate+' doublon(s) ticker/date détecté(s) dans l’échantillon analysé.']);
    ...
    host.innerHTML='<div class="card" style="margin-bottom:16px;"><div class="card-header"><div><span class="card-title">Contrôle qualité des séances</span><div style="font-size:11px;color:var(--muted);margin-top:4px;">Lecture seule · aucune donnée n’est modifiée automatiquement</div></div><button class="btn btn-outline btn-sm" onclick="loadHistoriqueQualityDashboard()">↺ Analyser</button></div><div class="kpi-row" style="padding:16px;">'+metric('Score global',a.health)+metric('Complétude',a.completeness)+metric('Propreté',a.cleanliness)+metric('Logique',a.logic)+metric('Données renseignées',a.missingScore)+'</div></div>'+\
      '<div class="card" style="margin-bottom:16px;"><div class="card-header"><span class="card-title">Couverture par séance</span><span style="margin-left:auto;font-size:11px;color:var(--muted);">Univers actif : '+pair[1].length+' titres</span></div><div style="padding:12px 18px 22px;">'+hqChart(a.dates)+'</div></div>'+\
      '<div class="card"><div class="card-header"><span class="card-title">Alertes de contrôle</span><span style="margin-left:auto;font-size:11px;color:var(--muted);">La décision de validation reste manuelle</span></div><div style="padding:10px 18px 18px;">'+alerts.map(function(x){var icon=x[0]==='critical'?'⛔':x[0]==='warning'?'⚠️':'✓';var color=x[0]==='critical'?'var(--red)':x[0]==='warning'?'var(--orange)':'var(--green)';return '<div style="display:flex;gap:10px;padding:12px 0;border-bottom:1px solid var(--border);"><span style="color:'+color+';font-size:15px;">'+icon+'</span><div><b style="font-size:12px;">'+hqEsc(x[1])+'</b><div style="font-size:11px;color:var(--muted);margin-top:3px;">'+hqEsc(x[2])+'</div></div></div>';}).join('')+'</div></div>'+\
      '<div class="card" style="margin-top:16px;"><div class="card-header"><span class="card-title">Détail des dernières séances</span></div><div class="tw"><table><thead><tr><th>Date</th><th class="r">Titres</th><th class="r">Complétude</th><th class="r">Manquants</th><th class="r">OHLC</th><th class="r">Variations extrêmes</th><th>État</th></tr></thead><tbody>'+a.dates.map(function(x){var state=x.invalid||x.ohlc?'ERREUR':x.coverage<90?'INCOMPLÈTE':x.coverage<100?'PARTIELLE':'OK';var color=state==='ERREUR'?'var(--red)':state==='INCOMPLÈTE'?'var(--red)':state==='PARTIELLE'?'var(--orange)':'var(--green)';return '<tr><td class="td-muted">'+hqEsc(x.date)+'</td><td class="r td-mono">'+x.tickers+'</td><td class="r td-mono">'+x.coverage.toFixed(1)+'%</td><td class="r td-mono">'+x.missing+'</td><td class="r td-mono">'+x.ohlc+'</td><td class="r td-mono">'+x.extremeVar+'</td><td style="color:'+color+';font-weight:600;">'+state+'</td></tr>';}).join('')+'</tbody></table></div></div>';
