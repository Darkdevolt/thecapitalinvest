...
function sbHeaders(extra) {
    const base = { apikey: CONFIG.SB_ANON, Authorization: 'Bearer ' + TK, 'Content-Type': 'application/json', Prefer: 'return=representation' };
    if (extra) {
...
async function sbGet(table, params) {
    try {
        const url = CONFIG.SB_REST + '/' + table + (params ? '?' + params : '');
...
async function sbCount(table, params) {
    try {
        ...
        const url = CONFIG.SB_REST + '/' + table + '?select=*&limit=0' + (cleanParams ? '&' + cleanParams : '');
        const ctrl = new AbortController();
...
async function sbPost(table, body, onConflict) {
    try {
        ...
        let url    = onConflict ? CONFIG.SB_REST + '/' + table + '?on_conflict=' + onConflict : CONFIG.SB_REST + '/' + table;
        ...
        if (!r.ok) {
            ...
            let errMsg = e && e.message || e && e.details || JSON.stringify(e);

            if (errMsg) {
                if (errMsg.indexOf('foreign key') !== -1) {
                    const t = body && body.ticker ? ' "' + body.ticker + '"' : '';
                    ...
                    const t = body && body.ticker ? ' pour ' + body.ticker : '';
                    const d = body && body.date_seance ? ' du ' + body.date_seance : (body && body.annee ? ' ' + body.annee : '');
...
async function sbPatch(table, filter, body) {
    try {
        const ctrl = new AbortController();
        const t = setTimeout(function() { ctrl.abort(); }, 10000);
        const r = await fetch(CONFIG.SB_REST + '/' + table + '?' + filter, { method:'PATCH', headers: sbHeaders(), body: JSON.stringify(body), signal: ctrl.signal });
        clearTimeout(t);
        if (!r.ok) {
            const e = await r.json().catch(function() { return {}; });
            let raw = e && e.message || e && e.details || 'Erreur MAJ';
        ...
        if (e.name !== 'AbortError') toast('Réseau
