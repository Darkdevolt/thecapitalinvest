...
(function () {
  ...
  /** Retire d'un enregistrement les colonnes absentes de la table. */
  ...
  function afficherControles(ecarts, contexte) {
    ...
    if (!hote) return;
    if (!ecarts.length) {
      hote.innerHTML = '<div class="fin-ok">Cohérence vérifiée : sous-totaux, équilibre du bilan '
        + 'et concordance du résultat net sont conformes' + (contexte ? ' (' + esc(contexte) + ')' : '') + '.</div>';
      return;
    }
    hote.innerHTML = '<div class="fin-ecarts"><strong>' + ecarts.length
      + ' écart(s) de cohérence</strong>'
      + '<p>Ces écarts sont signalés, pas corrigés : une publication comporte des arrondis et '
      + 'des reclassements légitimes. Vérifiez avant d\'enregistrer.</p><ul>'
      + ecarts.map(function (e) { return '<li>' + esc(e.message) + '</li>'; }).join('')
      + '</ul></div>';
  }

  function verifier() {
    afficherControles(S().controler(lireFormulaire()));
  }
  ...
  async function charger() {
    ...
    var rows = await sbGet('financials',
      'select=*&ticker=eq.' + encodeURIComponent(c.ticker)
      + '&annee=eq.' + c.annee + '&limit=1');
    if (!rows || !rows.length) {
      toast2('Aucun exercice enregistré pour ' + c.ticker + ' ' + c.annee + '.', 'warn');
      return;
    }
  ...
  async function enregistrer() {
    ...
    if (a.ignorees.length) {
      toast2('Colonnes absentes de la base, non enregistrées : ' + a.ignorees.join(', ')
        + '. La migration SYSCOHADA a-t-elle été exécutée ?', 'warn');
    }

    var res = await sbPost('financials', a.ligne, 'ticker,annee');
    if (res) {
      toast2('Exercice ' + a.ligne.ticker + ' ' + a.ligne.annee + ' enregistré.', 'ok');
      afficherControles(ecarts, a.ligne.ticker + ' ' + a.ligne.annee);
    }
  }
  ...
  async function exporterExcel() {
    ...
    try {
      var nom = window.FinancialsExcel.exporter(rows, { ticker: c.ticker });
      toast2('Classeur généré : ' + nom + ' (' + rows.length + ' exercice(s)).', 'ok');
    } catch (e) {
      toast2('Export impossible : ' + e.message, 'err');
    }
  }
  ...
  function importerExcel(fichier) {
    ...
    lecteur.onload = async function () {
      ...
      try {
        ...
        if (!r.lignes.length) {
          hote.innerHTML = '<div class="fin-ecarts">Aucun exercice exploitable dans ce fichier.</div>';
          return;
        }
        // Rien n'est écrit avant confirmation explicite : l'aperçu affiche ce
        // qui sera enregistré et ce qui pose question.
        hote.innerHTML =
          '<div class="fin-apercu"><strong>' + r.lignes.length + ' exercice(s) prêt(s) à l\'import</strong>'
          + '<div class="fin-apercu-liste">'
          + r.lignes.map(function (l) {
              return '<span class="fin-chip">' + esc(l.ticker) + ' ' + esc(l.annee) + '</span>';
            }).join('')
          + '</div>'
          + (r.anomalies.length
              ? '<p class="fin-warn">' + r.anomalies.length + ' avertissement(s) :</p><ul>'
                + r.anomalies.map(function (a) { return '<li>' + esc(a) + '</li>'; }).join('') + '</ul>'
              : '')
          + (r.ecarts.length
              ? '<p class="fin-warn">' + r.ecarts.length + ' écart(s) de cohérence :</p><ul>'
                + r.ecarts.slice(0, 12).map(function (e) {
                    return '<li>' + esc(e.annee) + ' — ' + esc(e.message) + '</li>';
                  }).join('') + '</ul>'
              : '<p class="fin-ok-inline">Aucun écart de cohérence détecté.</p>')
          + '<button type="button" class="btn" id="fin-import-confirmer">Enregistrer ces '
          + r.lignes.length + ' exercice(s)</button></div>';
        ...
        document.getElementById('fin-import-confirmer').addEventListener('click', async function () {
          ...
          toast2(ok + ' exercice(s) enregistré(s)'
            + (echecs ? ', ' + echecs + ' en échec' : '')
            + (restantes.length ? '. Colonnes absentes de la base : ' + restantes.join(', ') : '.'),
            echecs ? 'warn' : 'ok');
          if (typeof window.loadFinancials === 'function') window.loadFinancials();
        });
      } catch (e) {
        hote.innerHTML = '<div class="fin-ecarts">Lecture impossible : ' + esc(e.message) + '</div>';
      }
    };
  ...
  function jeton() {
    return (window.TC_ENV && window.TC_ENV.getToken()) || (typeof TK !== 'undefined' ? TK : '');
  }
  ...
  async function apiDocs(methode, corps, requete) {
    ...
    var r = await fetch('/api/financials-upload' + (requete || ''), {
      method: methode, headers: entetes,
      body: corps ? JSON.stringify(corps) : undefined, cache: 'no-store'
    });
  ...
  async function listerDocs() {
    ...
    try {
      ...
      if (!docs.length) {
        hote.innerHTML = '<div class="fin-vide">Aucun document déposé'
          + (c.ticker ? ' pour ' + esc(c.ticker) : '') + '.</div>';
        return;
      }
      hote.innerHTML = '<table class="fin-docs"><thead><tr>'
        + '<th>Ticker</th><th>Exercice</th><th>Type</th><th>Fichier</th><th>Visible</th><th></th>'
        + '</tr></thead><tbody>'
        + docs.map(function (d) {
            return '<tr><td class="td-gold">' + esc(d.ticker) + '</td>'
              + '<td>' + esc(d.annee) + ' · ' + esc(d.periode) + '</td>'
              + '<td>' + esc(String(d.type_document).replace(/_/g, ' ')) + '</td>'
              + '<td><a href="' + esc(d.fichier_url) + '" target="_blank" rel="noopener">'
              + esc(d.fichier_nom) + '</a>'
              + (d.taille_octets ? ' <span class="fin-taille">' + Math.round(d.taille_octets / 1024) + ' Ko</span>' : '')
              + '</td>'
              + '<td>' + (d.publie ? 'oui' : 'non') + '</td>'
              + '<td><button type="button" class="fin-suppr" data-id="' + esc(d.id) + '">Supprimer</button></td></tr>';
          }).join('')
        + '</tbody></table>';

      Array.prototype.forEach.call(hote.querySelectorAll('.fin-suppr'), function (b) {
        b.addEventListener('click', async function () {
          if (!confirm('Supprimer définitivement ce document ?')) return;
          try {
            await apiDocs('DELETE', null, '?id=' + encodeURIComponent(b.getAttribute('data-id')));
            toast2('Document supprimé.', 'ok');
            listerDocs();
          } catch (e) { toast2('Suppression impossible : ' + e.message, 'err'); }
        });
      });
    } catch (e) {
      hote.innerHTML = '<div class="fin-ecarts">Liste indisponible : ' + esc(e.message)
        + '. La table financials_documents existe-t-elle ?</div>';
    }
  }
  ...
  async function deposerDoc() {
    ...
    var meta = {
      ...
      type_document: (document.getElementById('fin-doc-type') || {}).value || 'etats_financiers',
      titre: (document.getElementById('fin-doc-titre') || {}).value || null,
      publie: !(document.getElementById('fin-doc-brouillon') || {}).checked
    };
    ...
    try {
      ...
      // Téléversement direct vers le stockage : le fichier ne transite pas par
      // la fonction serverless, dont la limite de charge utile est bien
      // inférieure à la taille d'un rapport annuel.
      poser('Téléversement (' + Math.round(fichier.size / 1024) + ' Ko)…');
      var up = await fetch(prep.signedUrl, {
        method: 'PUT', headers: { 'Content-Type': 'application/pdf' }, body: fichier
      });
      if (!up.ok) throw new Error('Téléversement refusé (HTTP ' + up.status + ')');

      poser('Enregistrement…');
      await apiDocs('POST', Object.assign({
        action: 'finalize', path: prep.path, filename: fichier.name, taille: fichier.size
      }, meta));
  ...
  function installer() {
    ...
    carte.innerHTML =
      '<div class="card-header"><span class="card-title">Saisie complète — plan SYSCOHADA</span></div>'
      + '<div class="fin-corps">' + construireFormulaire() + '</div>'

      + '<div class="card-header fin-sep"><span class="card-title">Modèle Excel</span></div>'
      + '<div class="fin-corps">'
      + '  <p class="fin-note">Le classeur comporte une feuille par état, postes en lignes et '
      + 'exercices en colonnes. Sa structure est engendrée à partir du même dictionnaire que ce '
      + 'formulaire : les deux ne peuvent pas diverger.</p>'
      + '  <div class="fin-actions">'
      + '    <button type="button" id="fin-excel-export" class="btn ghost">Exporter / modèle vierge</button>'
      + '    <label class="btn ghost" for="fin-excel-fichier">Importer un classeur</label>'
      + '    <input type="file" id="fin-excel-fichier" accept=".xlsx,.xls" hidden>'
      + '  </div>'
      + '  <div id="fin-import-apercu"></div>'
      + '</div>'

      + '<div class="card-header fin-sep"><span class="card-title">États financiers en PDF</span></div>'
      + '<div class="fin-corps">'
      + '  <p class="fin-note">Le document déposé est rattaché au ticker et à l\'exercice saisis '
      + 'ci-dessus. Publié, il devient consultable depuis la fiche de la valeur dans l\'application.</p>'
      + '  <div class="fin-doc-form">'
      + '    <select id="fin-doc-type" class="fin-input">'
      + '      <option value="etats_financiers">États financiers</option>'
      + '      <option value="rapport_annuel">Rapport annuel</option>'
      + '      <option value="rapport_semestriel">Rapport semestriel</option>'
      + '      <option value="communique">Communiqué</option>'
      + '      <option value="note_information">Note d\'information</option>'
      + '      <option value="autre">Autre</option>'
      + '    </select>'
      + '    <input type="text" id="fin-doc-titre" class="fin-input" placeholder="Titre (facultatif)">'
      + '    <input type="file" id="fin-doc-fichier" accept="application/pdf" class="fin-input">'
      + '    <label class="fin-check"><input type="checkbox" id="fin-doc-brouillon"> Ne pas publier</label>'
      + '    <button type="button" id="fin-doc-deposer" class="btn">Déposer</button>'
      + '  </div>'
      + '  <div id="fin-doc-msg" class="msg"></div>'
      + '  <div id="fin-docs-liste"></div>'
      + '</div>';

    panneau.insertBefore(carte, panneau.firstChild ? panneau.firstChild.nextSibling : null);

    var lier = function (id, ev, fn) {
      var el = document.getElementById(id);
      if (el) el.addEventListener(ev, fn);
    };
    lier('fin-charger', 'click', charger);
    lier('fin-vider', 'click', function () {
      remplirFormulaire(null);
      document.getElementById('fin-controles').innerHTML = '';
    });
    lier('fin-verifier', 'click', verifier);
    lier('fin-enregistrer', 'click', enregistrer);
    lier('fin-excel-export', 'click', exporterExcel);
    lier('fin-excel-fichier', 'change', function () {
      if (this.files && this.files[0]) importerExcel(this.files[0]);
    });
