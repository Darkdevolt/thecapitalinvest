...
(function () {
  ...
  function loadScript(src, done) {
    ...
    script.src = src; script.async = false; script.dataset.tcRuntime = src;
    script.onload = function () { if (typeof done === 'function') done(); };
    script.onerror = function () { console.error('[INIT] Script impossible à charger:', src); if (typeof done === 'function') done(); };
    document.head.appendChild(script);
  }

  function loadStyle(href) {
    if (document.querySelector('link[data-tc-runtime-style="' + href.replace(/"/g, '') + '"]')) return;
    var link = document.createElement('link'); link.rel = 'stylesheet'; link.href = href; link.dataset.tcRuntimeStyle = href;
  ...
  function loadRuntimeLayers() {
    ...
    loadStyle('app/css/fundamental-pro-enhancement.css?v=1');
        ...
        loadScript('app/js/views/user-data-patch.js?v=7', function () {
          loadScript('app/js/views/fundamental-ratios.js?v=1', function () {
            loadScript('app/js/views/fundamental-data-filters.js?v=20260816', function () {
              loadScript('app/js/views/fundamental-pro-enhancement.js?v=1', function () {
                renderAfterData();
              });
            });
          });
  ...
  function ensureSuiviNavigation() {
    ...
    if (sidebar && !sidebar.querySelector('[data-tc-suivi]')) {
      var sections = sidebar.querySelectorAll('.sidebar-section'); var gestionSection = null;
    ...
    if (menu && !menu.querySelector('[data-tc-suivi]')) {
      ...
      menu.appendChild(separator); menu.appendChild(desktopItem);
    }
  }
  ...
  function init() {
    ...
    normalizeDocument(); console.log('[INIT] Session authentifiée, démarrage The Capital');
