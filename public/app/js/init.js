...
(function () {
  ...
  function getSession() {
    try {
      ...
      if (!session || !tokenIsValid(session.access_token)) { localStorage.removeItem(SESSION_KEY); return null; }
      return session;
  ...
  function normalizeDocument() {
    ...
    document.querySelectorAll('a[href]').forEach(function (a) {
      ...
      if (href && href.charAt(0) !== '#' && !/^(https?:|mailto:|tel:|javascript:)/i.test(href)) { a.removeAttribute('target'); a.removeAttribute('rel'); }
    });
  }
  function loadScript(src, done) {
    var existing = document.querySelector('script[data-tc-runtime="' + src.replace(/"/g, '') + '"]');
    if (existing) { done(); return; }
    var script = document.createElement('script'); script.src = src; script.async = false; script.dataset.tcRuntime = src; script.onload = done;
  ...
  /* Mobile navigation: creates the button without altering the existing HTML/navigation handlers. */
  function initMobileMenu() {
    ...
    btn.onclick = function () { setMenu(!sidebar.classList.contains('mobile-open')); };
    if (overlay) overlay.onclick = function () { setMenu(false); };
    sidebar.querySelectorAll('.nav-item').forEach(function (item) {
      item.addEventListener('click', function () { if (window.innerWidth <= 760) setMenu(false); });
    });
