...
function atResolveCourseRow(requested){
  const q=atNormTicker(requested);if(!q)return null;
  const rows=Array.isArray(window.allCours)?window.allCours:[];
  let row=rows.find(r=>atRowTicker(r)===q);
  if(row)return row;
  row=rows.find(r=>atNormTicker(atRowName(r))===q);
  if(row)return row;
  const qCompact=q.replace(/[^A-Z0-9]/g,'');
  return rows.find(r=>{const t=atNormTicker(atRowTicker(r)).replace(/[^A-Z0-9]/g,'');const n=atNormTicker(atRowName(r)).replace(/[^A-Z0-9]/g,'');return t===qCompact||n===qCompact||t.includes(qCompact)||n.includes(qCompact)})||null;
}
function atEnsureTickerBridge(){
  if(window.__atTickerBridgeInstalled)return;
  if(typeof window.atLoadTicker!=='function')return;
  const original=window.atLoadTicker;window.__atTickerBridgeInstalled=true;
  window.atLoadTicker=async function(){
    const sel=document.getElementById('atTicker');const requested=sel?.value||window.AT?.ticker||'';const row=atResolveCourseRow(requested);const canonical=atRowTicker(row)||atNormTicker(requested);
    if(row&&sel&&canonical){
      let option=Array.from(sel.options).find(o=>atNormTicker(o.value)===canonical);
      if(!option){option=document.createElement('option');option.value=canonical;option.textContent=canonical+(atRowName(row)?' — '+atRowName(row):'');sel.appendChild(option)}
      sel.value=canonical;
    }
    const ok=await original();
    if(ok)return true;
    // Fallback strictement front-end : le cours déjà chargé dans allCours est affiché
    // même si l'historique détaillé n'est pas résolu par le ticker exact.
    const fallback=atResolveCourseRow(canonical||requested);const px=atRowPrice(fallback);
    if(fallback&&px>0&&window.AT){
      const date=fallback.date_seance||fallback.date||new Date().toISOString().slice(0,10);
      window.AT.ticker=canonical||requested;window.AT.hist=[{date,o:px,h:px,l:px,c:px,v:Number(fallback.volume||fallback.vol||0)||0}];window.AT.zoom={start:0,end:1};
      const tickerEl=document.getElementById('atOhlcTicker');if(tickerEl)tickerEl.textContent=window.AT.ticker;
      const meta=document.getElementById('atTickerMeta');if(meta)meta.textContent=`Cours actuel ${px.toLocaleString('fr-FR')} FCFA • historique détaillé non disponible pour ce code`;
      if(typeof atRender==='function')atRender();
      if(typeof window.toast==='function')window.toast(`Cours ${window.AT.ticker} chargé depuis les cours de marché.`,'success');
      return true;
    }
    return false;
  };
