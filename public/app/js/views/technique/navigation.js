...
function atLoadModernTheme(){
  if(!document.getElementById('atModernCss')){
    ...
    link.id='atModernCss';link.rel='stylesheet';link.href='/app/css/technique-modern.css';
    document.head.appendChild(link);
  }
}

function atInstallThemeToggle(){
  const view=document.getElementById('view-analyse-technique');
  const toolbar=view?.querySelector('.at-toolbar');
  if(!view||!toolbar||document.getElementById('atThemeToggle'))return;
  const btn=document.createElement('button');
  btn.id='atThemeToggle';btn.type='button';btn.className='at-theme-toggle';
  btn.innerHTML='<span>Mode clair</span><span class="at-switch" aria-hidden="true"></span>';
  btn.setAttribute('aria-label','Activer ou désactiver le mode clair');
  const saved=localStorage.getItem('tc-at-theme');
  const set=(light,save=true)=>{
    view.classList.toggle('at-light',light);
    btn.querySelector('span:first-child').textContent=light?'Mode sombre':'Mode clair';
    btn.setAttribute('aria-pressed',String(light));
    if(save)localStorage.setItem('tc-at-theme',light?'light':'dark');
    if(typeof atRender==='function'&&window.AT?.hist?.length)setTimeout(()=>atRender(),0);
  };
  btn.addEventListener('click',()=>set(!view.classList.contains('at-light')));
  toolbar.appendChild(btn);
  set(saved==='light',false);
}
