...
(function (global) {
  ...
  function buildView() {
    ...
    /* Heikin-Ashi est un mode d'affichage, pas une source de données : il
       est calculé sur la série complète puis découpé, et n'intervient ni
       dans l'analyse ni dans le bandeau de cotation, qui continuent de
       porter les prix réels. */
    if (S.type === 'heikin') {
      var ha = M.heikinAshi(full.o, full.h, full.l, full.c);
      visible = visible.map(function (b, i) {
        var k = range.from + i;
        return M.fin(ha.c[k])
          ? { date: b.date, o: ha.o[k], h: ha.h[k], l: ha.l[k], c: ha.c[k], v: b.v, count: b.count }
          : b;
      });
    }
    ...
    return {
      bars: visible,
      offset: range.from,
      type: S.type === 'heikin' ? 'candle' : S.type,
      heikin: S.type === 'heikin',
      interval: S.interval,
      log: S.log,
      light: S.light,
      overlays: overlays,
      ...
      livePrice: S.type === 'heikin' ? null : (M.fin(livePrice) && livePrice > 0 ? livePrice : null),
  ...
  var TYPES = [
    { v: 'candle', l: 'Chandeliers' }, { v: 'hollow', l: 'Creux' }, { v: 'heikin', l: 'Heikin-Ashi' }, { v: 'bar', l: 'Barres' },
