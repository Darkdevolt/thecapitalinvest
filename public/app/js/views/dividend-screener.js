...
(function () {
  ...
  async function renderDividendScreener() {
    ...
    view.innerHTML = `
      <div class="page-header"><h1>Dividend <span style="color:var(--gold)">Screener</span></h1><p>Filtrez les sociétés selon le rendement et la croissance du dividende, à partir de dividen​des_calendrier.</p></div>
    ...
    ['divMinYield','divMaxYield','divMinGrowth','divYear'].forEach(id => document.getElementById(id)?.addEventListener('input', apply));
    apply();
  }
  ...
  function apply() {
    ...
    rows.forEach(r => {
      ...
      if (!ticker) return;
      const y = n(r.exercice ?? r.annee);
      const current = byTicker.get(ticker);
      if (!current || Number(y || 0) > Number(current.exercice ?? current.annee ?? 0)) byTicker.set(ticker, {...r, ticker});
    });
    let out = Array.from(byTicker.values()).map(r => {
      const current = n(r.montant_net ?? r.montant);
      const yieldValue = n(r.rendement ?? r.taux_rendement);
      const previous = rows.filter(x => x.ticker === r.ticker && Number(x.exercice ?? x.annee) === Number((r.exercice ?? r.annee)) - 1)[0];
    ...
    }).filter(r => (r.yieldValue ?? -Infinity) >= minY && (r.yieldValue ?? Infinity) <= maxY && (minG == null || (r.growth != null && r.growth >= minG)) && (!year || String(r.exercice ?? r.annee) === year));
