...
(function () {
  ...
  const dividendYear = r => n(r?.annee);

  async function loadRows() {
    try {
      const data = await window.apiGet('/marche?type=dividendes');
      rows = (Array.isArray(data) ? data : []).map(r => ({ ...r, annee: dividendYear(r) }));
    } catch (e) {
  ...
  async function renderDividendScreener() {
    ...
    const years = [...new Set(rows.map(dividendYear).filter(Boolean))].sort((a,b) => b-a);

    view.innerHTML = `
      <div class="page-header"><h1>Dividend <span style="color:var(--gold)">Screener</span></h1><p>Filtrez les sociétés selon le rendement et la croissance du dividende. L'année correspond à l'exercice bénéficiaire.</p></div>
  ...
  function apply() {
    ...
    const sourceRows = selectedYear ? rows.filter(r => String(dividendYear(r)) === selectedYear) : rows;
    const byTicker = new Map();
    sourceRows.forEach(r => {
      const ticker = String(r.ticker || '').toUpperCase();
      if (!ticker) return;
      const year = dividendYear(r);
      const current = byTicker.get(ticker);
      if (!current || Number(year || 0) > Number(dividendYear(current) || 0)) byTicker.set(ticker, { ...r, ticker, annee: year });
    });
    ...
    const out = Array.from(byTicker.values()).map(r => {
      ...
      const currentYear = dividendYear(r);
      const previous = rows.find(x => String(x.ticker || '').toUpperCase() === r.ticker && dividendYear(x) === currentYear - 1);
    ...
    tbody.innerHTML = out.length
      ? out.sort((a,b) => (b.yieldValue ?? -Infinity) - (a.yieldValue ?? -Infinity)).map(r => `<tr><td><strong style="color:var(--gold)">${esc(r.ticker)}</strong></td><td>${esc(dividendYear(r) ?? '—')}</td><td class="right">${r.montant_net ?? r.montant ?? 'Donnée non disponible'}</td><td class="right">${pct(r.yieldValue)}</td><td class="right pro-only">${pct(r.growth)}</td><td>${esc(r.date_detachement ?? r.ex_date ?? '—')}</td><td>${esc(r.date_paiement_cal ?? r.date_paiement ?? '—')}</td></tr>`).join('')
