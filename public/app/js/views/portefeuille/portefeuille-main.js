// ═══════════════════════════════════════════════════════
// PORTEFEUILLE, POINT D'ENTRÉE (v4)
...
function refreshBuyPriceFromMarket(force = false) {
  ...
  if (selectedDate === today && (force || !priceEl.value || autoFilled)) {
    ...
    if (!hint) {
      ...
      hint.style.cssText = 'font-size:12px;color:var(--dim);margin-top:7px;line-height:1.5';
...
function pfActionButton(label, onclick, variant = 'default') {
  const border = variant === 'danger' ? 'var(--red)' : 'var(--border)';
  const color = variant === 'danger' ? 'var(--red)' : 'var(--cream)';
  return `<button type="button" class="pf-action-btn" onclick="${onclick}" style="min-width:82px;min-height:38px;padding:8px 12px;border:1px solid ${border};border-radius:8px;background:var(--panel2);color:${color};font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap">${label}</button>`;
}
...
window.renderPortfolio = function() {
  ...
  try {
    ...
    if (tbody) {
      if (!displayRows.length) tbody.innerHTML = `<tr><td colspan="12" style="text-align:center;padding:28px;color:var(--dim)">Aucun résultat pour « ${st.search} ».</td></tr>`;
        ...
        return `<tr>
          ...
          <td style="padding:14px 12px;min-width:190px">
            ...
            <details style="margin-top:9px">
              <summary style="display:inline-flex;align-items:center;gap:7px;min-height:34px;padding:7px 11px;border:1px solid var(--border);border-radius:8px;background:var(--panel2);color:var(--cream);font-size:12px;font-weight:600;cursor:pointer;list-style:none">Achats · ${p.cmpPositions.length}</summary>
              <div style="margin-top:7px;padding:8px 11px;border:1px solid var(--border2);border-radius:8px;background:var(--panel);max-width:330px">${purchaseHistory}</div>
            </details>
          </td>
          <td style="padding:14px 12px;text-align:right;font-size:14px">${typeof fmt === 'function' ? fmt(p.qty) : p.qty}</td>
          <td style="padding:14px 12px;text-align:right;font-family:var(--mono);color:var(--gold);font-size:14px;font-weight:600">${typeof fmt === 'function' ? fmt(p.cmp, 2) : p.cmp.toFixed(2)}</td>
          <td style="padding:14px 12px;text-align:right;color:${priceFound ? 'inherit' : 'var(--dim)'};font-size:14px;font-weight:600">${typeof fmt === 'function' ? fmt(p.currentPrice, 2) : p.currentPrice.toFixed(2)}${!priceFound ? ' <small>(estimé)</small>' : ''}</td>
          <td style="padding:14px 12px;text-align:right;color:${p.pl >= 0 ? 'var(--green)' : 'var(--red)'};font-weight:700;font-size:14px">${p.pl >= 0 ? '+' : ''}${typeof fmtM === 'function' ? fmtM(p.pl) : p.pl.toFixed(0)}</td>
          ...
          <td style="padding:14px 12px;text-align:center;min-width:285px">
            <div class="row-actions" style="display:flex;justify-content:center;gap:8px;flex-wrap:wrap">
              ...
              ${pfActionButton('Vendre', `openSellModal('${p.ticker}')`)}
              ${pfActionButton('Supprimer', `removePosition(${p.id})`, 'danger')}
            </div>
          </td>
        </tr>`;
      }).join('');
    ...
    if (divEl) { const totalDividends = dividends.reduce((s, d) => s + (+d.amount || 0), 0); divEl.innerHTML = `<div style="padding:18px"><div style="display:flex;justify-content:space-between;margin-bottom:14px;font-size:13px"><span style="color:var(--dim)">Total dividendes perçus</span><span style="color:var(--gold);font-weight:700">${typeof fmtM === 'function' ? fmtM(totalDividends) : totalDividends.toFixed(0)} FCFA</span></div>${dividends.length ? dividends.slice(-5).reverse().map(d => `<div style="display:flex;justify-content:space-between;font-size:13px;padding:7px 0;border-bottom:1px solid var(--border2)"><span>${d.ticker}, ${typeof fmtDate === 'function' ? fmtDate(d.date) : d.date}</span><span style="color:var(--green)">+${typeof fmtM === 'function' ? fmtM(d.amount) : d.amount}</span></div>`).join('') : '<div style="font-size:13px;color:var(--dim)">Aucun dividende enregistré</div>'}</div>`; }
...
function renderWatchlist() { const el = document.getElementById('watchlistPanel'); if (!el) return; const list = getWatchlist(); if (!list.length) { el.innerHTML = '<div style="grid-column:1/-1;text-align:center;color:var(--dim);font-size:13px;padding:18px">Aucun titre suivi.</div>'; return; } el.innerHTML = list.map(w => { const price = getLatestPrice(w.ticker); return `<div class="watchlist-card"><button class="wl-remove" onclick="removeFromWatchlist('${w.ticker}')" title="Retirer">Retirer</button><div class="wl-ticker">${w.ticker}</div><div class="wl-price">${price ? (typeof fmt === 'function' ? fmt(price, 2) : price.toFixed(2)) + ' FCFA' : 'Cours indisponible'}</div><div style="font-size:11px;color:var(--dim)">Ajouté le ${typeof fmtDate === 'function' ? fmtDate(w.addedAt) : w.addedAt}</div></div>`; }).join(''); }
function renderAlerts() { const el = document.getElementById('alertsPanel'); if (!el) return; const alerts = getAlerts(); if (!alerts.length) { el.innerHTML = '<div style="text-align:center;color:var(--dim);font-size:13px;padding:18px">Aucune alerte active.</div>'; return; } el.innerHTML = alerts.map(a => { const price = getLatestPrice(a.ticker); const triggered = price != null && ((a.condition === 'above' && price >= a.target) || (a.condition === 'below' && price <= a.target)); return `<div class="alert-item ${triggered ? 'triggered' : ''}" style="min-height:48px;font-size:13px"><span>${triggered ? 'Alerte' : 'En attente'} · <b style="color:var(--gold)">${a.ticker}</b> ${a.condition === 'above' ? 'au-dessus de' : 'en dessous de'} ${typeof fmt === 'function' ? fmt(a.target, 2) : a.target} FCFA ${price != null ? `<span style="color:var(--dim)"> · actuel ${typeof fmt === 'function' ? fmt(price, 2) : price.toFixed(2)}</span>` : ''} ${triggered ? '<span class="badge badge-gold" style="margin-left:6px">Déclenchée</span>' : ''}</span><button onclick="removePriceAlert(${a.id})" style="min-width:42px;min-height:36px;background:var(--panel2);border:1px solid var(--border);border-radius:7px;color:var(--dim);cursor:pointer">Supprimer</button></div>`; }).join(''); }
function renderGoal(totalWithCash) { const el = document.getElementById('goalSummary'); if (!el) return; const goal = getGoal(); if (!goal || !goal.target) { el.innerHTML = '<div style="text-align:center;color:var(--dim);font-size:13px;padding:18px">Aucun objectif défini pour l\'instant.</div>'; return; } const progress = Math.min(100, (totalWithCash / goal.target) * 100); el.innerHTML = `<div style="display:flex;justify-content:space-between;font-size:13px;color:var(--dim)"><span>Progression vers ${typeof fmtM === 'function' ? fmtM(goal.target) : goal.target} FCFA</span><span style="color:var(--gold);font-weight:700">${typeof fmt === 'function' ? fmt(progress, 1) : progress.toFixed(1)}%</span></div><div class="goal-progress-track"><div class="goal-progress-fill" style="width:${progress}%"></div></div>${goal.date ? `<div style="font-size:12px;color:var(--dim)">Échéance : ${typeof fmtDate === 'function' ? fmtDate(goal.date) : goal.date}</div>` : ''}`; }
function renderTransactionHistory() { const el = document.getElementById('transactionHistoryPanel'); if (!el) return; const txs = getTransactions().sort((a, b) => new Date(b.date_transaction || b.date || 0) - new Date(a.date_transaction || a.date || 0)).slice(0, 30); if (!txs.length) { el.innerHTML = '<div style="text-align:center;color:var(--dim);font-size:13px;padding:20px">Aucune transaction enregistrée.</div>'; return; } const labels = { ACHAT: 'ACHAT', VENTE: 'VENTE', DIVIDENDE: 'DIVIDENDE', DEPOT: 'DÉPÔT', RETRAIT: 'RETRAIT', buy: 'ACHAT', sell: 'VENTE', dividend: 'DIVIDENDE', deposit: 'DÉPÔT', withdraw: 'RETRAIT' }; el.innerHTML = txs.map(t => { const date = t.date_transaction || t.date; const qty = Number(t.quantite ?? t.qty ?? 0); const price = Number(t.cours ?? t.prix_unitaire ?? t.price ?? 0); const amount = Number(t.montant_net ?? t.amount ?? 0); const amountLabel = ['ACHAT','VENTE','buy','sell'].includes(t.type) ? `${qty} × ${typeof fmt === 'function' ? fmt(price, 2) : price} FCFA` : `${typeof fmtM === 'function' ? fmtM(amount) : amount} FCFA`; return `<div class="tx-item" style="min-height:44px;font-size:13px"><span style="color:var(--dim)">${typeof fmtDate === 'function' ? fmtDate(date) : date}</span><span class="tx-type tx-${String(t.type).toLowerCase()}">${labels[t.type] || t.type}</span><span>${t.ticker ? `<b style="color:var(--gold)">${t.ticker}</b>, ` : ''}${amountLabel}</span><span></span></div>`; }).join(''); }
function resetEmptyState() { const _money = { pfTotal: 1, pfInvested: 1 }; ['pfTotal','pfInvested','pfPL','pfReturn','pfVolatility','pfSharpe','pfDrawdown','pfBestPos','pfWorstPos'].forEach(id => { const el = document.getElementById(id); if (el) el.textContent = _money[id] ? '0 FCFA' : '—'; }); const cash = getCash(); const pfCash = document.getElementById('pfCash'); if (pfCash) pfCash.textContent = (typeof fmtM === 'function' ? fmtM(cash) : cash.toFixed(0)) + ' FCFA (liquide)'; const tbody = document.getElementById('pfTable'); if (tbody) tbody.innerHTML = `<tr><td colspan="12" style="text-align:center;padding:36px;color:var(--dim)"><div style="font-size:18px;margin-bottom:12px">Portefeuille vide</div><div style="font-size:13px;margin-bottom:20px">Ajoutez une position pour suivre vos investissements BRVM.</div><button onclick="document.getElementById('pfTicker').focus()" style="padding:12px 24px;background:var(--gold);border:none;border-radius:8px;color:var(--bg);font-weight:700;font-size:13px;cursor:pointer">Ajouter une position</button></td></tr>`; const countEl = document.getElementById('pfPositionCount'); if (countEl) countEl.textContent = '0 position · Ajoutez des titres BRVM'; [pfValueChartInst,pfSectorChartInst,pfGeoChartInst,pfPLChartInst].forEach(chart => { if (chart) chart.destroy(); }); pfValueChartInst = pfSectorChartInst = pfGeoChartInst = pfPLChartInst = null; const concEl=document.getElementById('concentrationStats'); if(concEl) concEl.innerHTML='<div style="text-align:center;color:var(--dim);font-size:13px;padding:20px">Ajoutez des positions pour calculer la concentration.</div>'; const divEstEl=document.getElementById('dividendStats'); if(divEstEl) divEstEl.innerHTML='<div style="text-align:center;color:var(--dim);font-size:13px;padding:20px">Ajoutez des positions pour calculer les dividendes.</div>'; const benchEl=document.getElementById('benchmarkStats'); if(benchEl) benchEl.innerHTML='<div style="text-align:center;color:var(--dim);font-size:13px;padding:20px">Ajoutez des positions pour comparer au benchmark.</div>'; const corrEl=document.getElementById('correlationMatrix'); if(corrEl) corrEl.innerHTML='<div style="text-align:center;color:var(--dim);font-size:13px;padding:20px">Ajoutez au moins 2 positions pour calculer les corrélations.</div>'; const rebalEl=document.getElementById('rebalancingPanel'); if(rebalEl) rebalEl.innerHTML='<div style="text-align:center;color:var(--dim);font-size:13px;padding:20px">Définissez des allocations cibles pour voir les suggestions.</div>'; }
