(function(){
  ...
  function getToken(){
    ...
    try{const stored=JSON.parse(localStorage.getItem('tc_session')||'null');const session=(stored&&stored.data&&stored.data.session)||(stored&&stored.session)||stored;if(session&&session.access_token)return session.access_token;}catch(e){}
  ...
  async function request(method,endpoint,body,options){
    ...
    try{return await promise;}finally{if(key&&inflight.get(key)===promise)inflight.delete(key);}
  }
