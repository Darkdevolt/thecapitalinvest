...
(function(){
  ...
  function getToken(){
    if(window.tcAccessToken)return window.tcAccessToken;
    ...
    }catch(e){console.warn('[API] Session illisible:',e);}
    return '';
  }

  async function request(method,endpoint,body,options){
    const url=normalizeEndpoint(endpoint),opts=options||{},token=getToken();
    const headers=Object.assign({'Accept':'application/json'},body!==undefined?{'Content-Type':'application/json'}:{},token?{'Authorization':'Bearer '+token}:{},opts.headers||{});
    ...
    try{
      ...
      if(method==='GET'&&cached){
        console.warn('[API] Données en cache utilisées après échec réseau:',endpoint);
        window.dispatchEvent(new CustomEvent('tc:stale-cache',{detail:{endpoint,error:err,cached:true}}));
        if(typeof window.toast==='function')window.toast('Connexion indisponible : données en cache affichées','info');
        return cached;
      }
  ...
  function loadTitleFixes(){
    ...
    const s=document.createElement('script');s.id='tc-title-fixes-script';s.src='/app/js/views/titres-navigation-fixes.js?v=20260815';s.async=false;
    s.onload=()=>console.log('[FETCH] Patch Titres/Fiche chargé');s.onerror=()=>console.warn('[FETCH] Patch Titres/Fiche indisponible');
