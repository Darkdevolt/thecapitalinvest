...
function loadEnv(){
  return new Promise(function(resolve){
    ...
    var done=false;
    var timer=setTimeout(function(){if(done)return;done=true;ENV=window.TC_ENV||null;resolve(false)},API_TIMEOUT);
    var s=document.createElement('script');
    s.src='/js/env.js';
    s.onload=function(){if(done)return;done=true;clearTimeout(timer);ENV=window.TC_ENV||null;resolve(!!ENV)};
    s.onerror=function(){if(done)return;done=true;clearTimeout(timer);resolve(false)};
...
async function api(path,options){
  ...
  var o=Object.assign({},options||{}),controller=new AbortController();
  var timer=setTimeout(function(){controller.abort()},API_TIMEOUT);
  o.headers=Object.assign({apikey:ENV.SUPABASE_ANON_KEY,Authorization:'Bearer '+token(),Accept:'application/json'},o.headers||{});
  o.signal=controller.signal;
  try{
    var r=await fetch(ENV.SUPABASE_URL+path,o),data=await r.json().catch(function(){return null});
  ...
  }catch(e){return {ok:false,kind:e&&e.name==='AbortError'?'timeout':'network',status:0,data:null,error:e}}
...
async function checkEnrollment(){
  ...
  if(!result.ok){enrolled=false;enrollmentState='unavailable';console.warn('[TCI] Vérification abonnement indisponible:',result.kind,result.status||'');return}
  var d=result.data;
...
function userState(){
  ...
  var next=enrollmentState==='active'?'✓ INSTITUTE ACTIF':enrollmentState==='inactive'?'✦ FORMATION':'⚠ Vérification indisponible';
  if(s.textContent!==next)s.textContent=next;
  if(s.getAttribute('data-tci-state')!==enrollmentState)s.setAttribute('data-tci-state',enrollmentState);
}
...
async function backgroundSync(){
  ...
  await checkEnrollment();userState();integrationStatus();if(enrolled)await pullProgress();userState();
}
...
function ready(){
  initializeUI();
  backgroundSync().catch(function(e){enrollmentState='unavailable';console.error('[TCI] Initialisation distante échouée:',e);userState();integrationStatus()});
}
/* Aucun MutationObserver global : userState() modifie le DOM et un observer documentElement créait une boucle auto-déclenchée. */
