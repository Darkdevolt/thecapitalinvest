...
var endpoint='/api/marche?type=apercu',nf=new Intl.NumberFormat('fr-FR',{maximumFractionDigits:2}),integer=new Intl.NumberFormat('fr-FR',{maximumFractionDigits:0});
...
loading();
fetch(endpoint,{headers:{Accept:'application/json'},credentials:'same-origin',cache:'force-cache',priority:'high'}).then(function(res){if(!res.ok)throw Error('HTTP '+res.status);return res.json()}).then(function(payload){var data=payload&&payload.text?JSON.parse(payload.text):payload;if(!data||data.success===false)throw Error('API BRVM indisponible');render(data)}).catch(function(err){console.warn('[TC Landing Market]',err);fallback()});
})();
