 ...
 */
async function scrapeEmetteur(slug, cutoffDate, maxPages) {
  const rows = [];
  for (let page = 0; page < maxPages; page++) {
    let html;
    try { html = await fetchHtml(`${BASE_URL}/${slug}${page ? `?page=${page}` : ''}`); }
    catch (error) {
...
export async function scrapeRapports({ slugs, sinceYears = 2, maxPages = MAX_PAGES, concurrency = 6 } = {}) {
  ...
  const list = (slugs && slugs.length ? slugs : Object.keys(EMETTEURS)).filter(s => EMETTEURS[s]);
  ...
  async function worker() {
    while (next < list.length) {
      ...
      try {
        const found = await scrapeEmetteur(slug, cutoffDate, maxPages);
