    ...
    for (const m of unitTrials) {
      ...
      for (const [k, cell] of cells) {
        ...
        for (const [f, v] of Object.entries(cell.fields)) {
          ...
          else if (!close(v * m, dbv, 0.08)) mismatches++;
        }
      }
      if (matches && !mismatches && (!chosen || matches > chosen.matches)) chosen = { m, matches, hit, how: 'recoupé' };
    }
    if (!chosen && anyRef) {
      report.push({ page: cand.page, ticker, status: 'rejected', reason: 'valeurs contredisant la base (aucune unité cohérente)' });
      continue;
    }
...
export function bocTelegramText({ bocDate, pagesTotal, pagesScanned, inserted, promotions, report }) {
  ...
  if (issues.length) lines.push(`⚠️ ${issues.map(r => `${r.ticker || 'p. ' + r.page}${r.annee ? ` ${r.periode} ${r.annee}` : ''} : ${r.reason}`).join(' ; ')}`);
  if (!inserted.length && !promotions.length && !issues.length) lines.push('Aucune nouvelle donnée financière.');
  return lines.join('\n');
}
