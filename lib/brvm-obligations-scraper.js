...
export async function scrapeBrvmObligations(dateOverride) {
  // cours-obligations/0 s'est montré plus instable que cours-actions en
  // production (délai de 25 s dépassé par intermittence, y compris sur des
  // passages où la séance elle-même a réussi). Une retentative courte est
  // sans risque ici : cet import tourne désormais en parallèle de la séance
  // (api/process-brvm.js, scope=auto) plutôt qu'avant elle, donc sa durée ne
  // grignote plus le budget d'exécution de 60 s de la partie critique.
  const html = await fetchHtml(SOURCE, { retries: 1, retryDelayMs: 1500 });
