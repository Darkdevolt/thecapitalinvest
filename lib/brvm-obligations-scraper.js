...
function parseBfinBondTrading(html, sessionDate) {
  ...
  for (const cells of tableRows(html)) {
    ...
    const [coursPrecedent, cours, volume, transactions, valeur] = values;
    if (cours == null) continue;
    const taux = (nom.match(/(\\d{1,2}(?:[.,]\\d{1,2})?)\\s*%/) || [])[1];
    const years = yearsFromName(nom);
    rows.push({
      code, nom,
      date_emission: years.emission,
      date_maturite: years.maturite,
      cours,
      coupon_couru: null,
      dernier_paiement_date: null,
      dernier_paiement_valeur: null,
      taux_facial: taux ? num(taux) : null,
      date_seance: sessionDate,
      volume: volume == null ? null : Math.round(volume),
      transactions: transactions == null ? null : Math.round(transactions),
      valeur_transigee: valeur,
      source_fallback: 'BRVM-BFIN'
    });
  }
...
export async function scrapeBrvmObligations(dateOverride) {
  ...
  let fallback = false;
  try {
    html = await fetchHtml(SOURCE, { retries: 1, retryDelayMs: 1500 });
  } catch (primaryError) {
    html = await fetchHtml(FALLBACK_SOURCE, { retries: 0 });
