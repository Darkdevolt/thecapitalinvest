...
const config = {
  ...
  // Secret dédié au déclencheur d'import automatique (pg_cron Supabase ->
  // en-tête x-tc-auto). Indépendant de CRON_SECRET pour ne pas toucher au
  // cron existant. Vide = déclencheur auto désactivé.
  autoCronSecret: process.env.AUTO_CRON_SECRET || '',
  ...
  // Bulletin de marché automatique envoyé sur Telegram (api/telegram-report.js).
  // Vide = fonctionnalité désactivée sans erreur, plutôt que de planter.
