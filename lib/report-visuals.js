...
let LOGO = null;
function logo() {
  if (!LOGO) LOGO = 'data:image/png;base64,' + asset('../public/assets/the-capital-logo.png').toString('base64');
  return LOGO;
}
...
function entete(meta, size = 1) {
  return h('div', { alignItems: 'center', gap: 22 * size },
    img(logo(), 96 * size, 96 * size),
...
function slideCadre(meta, n, total, ...body) {
  ...
  return fond(W, H,
    h('div', { position: 'absolute', top: 64, left: pad, right: pad, justifyContent: 'space-between', alignItems: 'center' },
      h('div', { alignItems: 'center', gap: 16 }, img(logo(), 64, 64), txt('THE CAPITAL', { fontSize: 22, fontWeight: 700, letterSpacing: 5, color: C.goldHi })),
