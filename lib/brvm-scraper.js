...
/**
 * `retries` vaut 0 par défaut : le comportement des appelants existants (pages
 * ESV/DC-BR/annonces, parcourues en boucle avec leur propre tolérance à
 * l'échec par page) ne change pas. Les deux sources critiques et à échec
 * fréquent — cours-actions (séance) et cours-obligations — l'activent
 * explicitement (voir scrapeBrvm() et brvm-obligations-scraper.js) : elles ne
 * se partagent plus le budget d'exécution de 60 s depuis que l'appelant
 * (api/process-brvm.js, scope=auto) les lance en parallèle plutôt qu'en
 * série, ce qui laisse la marge nécessaire pour retenter une fois avant
 * d'abandonner, au lieu de dépendre uniquement du cron qui retente toutes les
 * 15 minutes.
 */
async function fetchHtml(url, { timeoutMs = FETCH_TIMEOUT_MS, retries = 0, retryDelayMs = 2000 } = {}) {
  let lastError;
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      lastError = e?.name === 'AbortError'
        ? new Error(`${url} : délai dépassé (${timeoutMs} ms)`)
        : e;
    } finally {
      clearTimeout(timer);
    }
...
function parseCourses(html, sessionDate) {
  ...
  for (const cells of tableRows(html)) {
    ...
    // La variation est repérée par son signe % plutôt que par sa position brute :
    // une colonne en plus ou en moins ailleurs dans la ligne (valeur suspendue,
    // cours précédent absent pour une introduction récente, mise en forme
    // différente...) décalait sinon silencieusement clôture et variation sur la
    // mauvaise cellule pour certains titres — sans que rien d'anormal ne se soit
    // produit sur le marché, ce que le contrôle de cohérence en aval (variation
    // publiée vs recalculée, api/process-brvm.js) finissait par signaler comme
    // faux positif et bloquait toute l'écriture de la séance.
    const numeric = cells.slice(found.index + 1)
      .filter(cell => /^[-+]?\s*\d[\d\s.,]*%?$/.test(cell.trim()))
      .map(cell => ({ value: num(cell), percent: cell.includes('%') }));
    const pctIndex = numeric.findIndex(c => c.percent);
    const base = pctIndex >= 0 ? numeric.filter((_, i) => i !== pctIndex) : numeric;
    if (base.length < 4) continue;
    const [volume, , open, close] = base.map(c => c.value);
    const variation = pctIndex >= 0 ? numeric[pctIndex].value : (numeric.length >= 5 ? numeric[4].value : null);
...
async function optional(url, opts) {
  try { return { ok: true, html: await fetchHtml(url, opts) }; }
  catch (error) { return { ok: false, error: String(error?.message || error) }; }
}

export async function scrapeBrvm() {
  // « cours » est la seule source non facultative (voir plus bas) : une
  // retentative courte avant d'abandonner évite qu'un simple ralentissement
  // passager de brvm.org bloque toute la séance en attendant le prochain
  // passage du cron, 15 minutes plus tard.
  const [coursR, resumeR, capR, volR] = await Promise.all([
    optional(SOURCES.cours, { retries: 1, retryDelayMs: 1500 }), optional(SOURCES.resume),
...
export { SOURCES, clean, num, tableRows, fetchHtml, extractDate };
