...
// Dernière séance historique réellement échangée (volume > 0). Écarte les
// séances « placeholder » (clôture = veille, volume 0) insérées avant la
// publication officielle, qui affichaient toutes les valeurs à 0 %.
async function latestTradedDate() {
  const { data, error } = await db
    .from('historique')
    .select('date_seance')
    .gt('volume', 0)
