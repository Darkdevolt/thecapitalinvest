 ...
 * @returns {Promise<{rows: object[], errors: Array<{url:string, error:string}>, hasMore: boolean}>}
 */
export async function scrapeDcbrFiches({ categorie = null, maxPages = DEFAULT_MAX_PAGES } = {}) {
  ...
  for (const link of uniqueLinks) {
    try {
      ...
      const row = parseFicheDetail(html, link.url);
      if (row) {
        /* Le classement vient du filtre DC/BR utilisé pour la liste (fiable),
           pas d'une déduction sur le contenu de la fiche elle-même. */
