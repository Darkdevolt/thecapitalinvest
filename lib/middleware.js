...
export function isMachineRequest(req) {
  ...
  const auto = String(header(req, 'x-tc-auto') || '');
  if (config.cronSecret && (auth === `Bearer ${config.cronSecret}` || internal === config.cronSecret)) return true;
  // Déclencheur d'import automatique (pg_cron Supabase).
  if (config.autoCronSecret && auto === config.autoCronSecret) return true;
  return false;
}
