...
export const validators = {
  ...
  // FIX Q-02 : validation de format date YYYY-MM-DD
  date: (v) => typeof v === 'string' && DATE_REGEX.test(v) && !isNaN(Date.parse(v)),
};
...
export function validate(data, schema) {
  ...
  for (const [field, rules] of Object.entries(schema)) {
    ...
    if (!valid) {
      ...
      sanitized[field] = rules.type === 'ticker' ? v.toUpperCase() : v;
    }
  }
