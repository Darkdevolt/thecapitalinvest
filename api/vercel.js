...
async function handleBrvmSync(requestUrl, req){
  ...
  // scrape-brvm has JWT verification enabled, so use the server-only Supabase secret key.
  const supabaseKey=process.env.SUPABASE_SECRET_KEY||process.env.SUPABASE_SERVICE_ROLE_KEY||'';
  if(!supabaseUrl||!supabaseKey)return jsonResponse({success:false,error:'Supabase URL/secret key not configured'},503,'no-store');
  try{
