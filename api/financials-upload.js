 ...
 *
 * Même mécanique en deux temps que le dépôt des bulletins BOC : le serveur
 * délivre une URL signée, le navigateur téléverse directement vers le stockage,
 * puis le serveur enregistre la référence. Le fichier ne transite jamais par la
 * fonction serverless — un rapport annuel pèse couramment plusieurs mégaoctets,
 * bien au-delà de ce qu'une requête vers une fonction supporte confortablement.
 *
 * Les chiffres saisis dans `financials` ne remplacent pas ces documents : le
 * rapport porte les notes annexes, les méthodes comptables et l'opinion du
 * commissaire aux comptes, qu'aucune grille de saisie ne restitue.
 */
...
const TYPES = new Set([
  ...
  'communique', 'note_information', 'autre'
]);
...
/** Un exercice ne peut pas être postérieur à l'année en cours. */
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      let q = supabaseAdmin.from('financials_documents').select('*')
    ...
    if (req.method === 'DELETE') {
      ...
      const { data: doc, error: lookupError } = await supabaseAdmin
        .from('financials_documents').select('fichier_url').eq('id', id).maybeSingle();
    ...
    if (body?.action === 'prepare') {
      ...
      if (!/\.pdf$/i.test(filename)) {
        return fail(res, 400, 'Seuls les fichiers PDF sont acceptés.', 'INVALID_FILE_TYPE');
      }
    ...
    if (body?.action === 'finalize') {
      if (!body.path || !body.filename) {
        return fail(res, 400, 'Informations de dépôt incomplètes.', 'INCOMPLETE_PAYLOAD');
      }
      const record = {
